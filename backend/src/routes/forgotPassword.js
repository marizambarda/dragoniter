...
router.post('/forgot_password/request_code', async function(req, res){
  ...
  await db.query('UPDATE users SET forgot_password_code = ? WHERE email = ?', [code, req.body.email])

  res.status(204).end()
})

router.post('/forgot_password/verify_code', async function(req, res){
  const [users] = await db.query('SELECT * FROM users WHERE email = ? AND forgot_password_code = ? AND  forgot_password_code IS NOT NULL', [req.body.email, req.body.forgot_password_code])
  if(users.length > 0){
    res.status(204).end()
    return
  } 
...
router.post('/forgot_password/reset_password', async function(req, res){
  ...
  await db.query('UPDATE users SET password = ?, forgot_password_code = NULL WHERE id = ?', [passwordHash, users[0].id])
  res.json(users[0])
})
