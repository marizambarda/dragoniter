...
router.post('/posts', requireAuthentication, async function (req, res) {
  const [
    ...
    'INSERT INTO posts(user_id, body, created_at) VALUES(?, ?, NOW())',
...
router.get('/users/:nickname/posts', async function (req, res) {
  ...
  const [
    ...
    'SELECT posts.*, users.name, users.nickname, users.avatar_url FROM posts INNER JOIN users ON posts.user_id = users.id WHERE user_id = ? ORDER BY created_at DESC',
    [users[0].id]
  );
  res.json(posts);
});
...
router.delete('/post/:id', requireAuthentication, async function (req, res) {
  let [
    posts,
  ] = await db.query('SELECT * FROM posts WHERE id = ? AND user_id = ?', [
    req.params.id,
    req.user.id,
  ]);
  if (posts.length === 0) {
